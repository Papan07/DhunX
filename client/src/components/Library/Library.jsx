...
const Library = () => {
  ...
  const fetchLibraryData = async () => {
    ...
    try {
      ...
      const playlistsResponse = await axios.get('http://localhost:5000/api/library/playlists', { headers });
      if (playlistsResponse.data.success) {
        setPlaylists(playlistsResponse.data.data.playlists);
      }

      // Fetch liked songs
      const likedSongsResponse = await axios.get('http://localhost:5000/api/library/liked-songs', { headers });
